...
const StockSearchLayoutUseSWR = ({ userProfile }: { userProfile: UserProfile }) => {
  ...
  const { data, isLoading, isValidating } = useSWR(['/api/edgeGetRandomStuff', enc], ([url, enc]) => fetchData(url, enc))

  const RenderDisplay = ({ result }: { result: StockQuote[] }) => {
    console.log('stocks found: ', result.length)
  ...
  return (
    <>
      ...
      {isLoading && <BackdropLoader />} {data && <RenderDisplay result={data} />}
    </>
  )
}
